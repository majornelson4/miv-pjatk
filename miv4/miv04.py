...
model_response = x ** 5 * v_model2[0] + x ** 4 * v_model2[1] + x ** 3 * v_model2[2] + x ** 2 * v_model2[3] + x * \
                 v_model2[4] + v_model2[5]
plt.plot(x, model_response, 'g-')

# verifying quality of the model 2 using training and testing sets

e_train = (y_train - (
            x_train ** 5 * v_model2[0] + x_train ** 4 * v_model2[1] + x_train ** 3 * v_model2[2] + x_train ** 2 *
            v_model2[3] + x_train * v_model2[4] + v_model2[5]))
error_mean_train_model2 = e_train.T @ e_train / len(e_train)
e_test = (y_test - (
            x_test ** 5 * v_model2[0] + x_test ** 4 * v_model2[1] + x_test ** 3 * v_model2[2] + x_test ** 2 * v_model2[
        3] + x_test * v_model2[4] + v_model2[5]))
